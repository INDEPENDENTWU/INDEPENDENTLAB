...
function analyze(a){
  ...
  const runSim=[],gapSim=new Map(tokens.map(t=>[t,[]])),clusterSim=new Map(tokens.map(t=>[t,[]]));
  for(let k=0;k<samples;k++){const s=shuffled(a,rng);runSim.push(longestRun(s).len);for(const t of tokens){gapSim.get(t).push(longestGap(s,t).len);if(w)clusterSim.get(t).push(bestCluster(s,t,w).count)}}
  const found=[];
  const runCut=quantile(runSim,.96);if(run.len>=3&&run.len>=runCut)found.push({type:'run',start:run.start,end:run.end,title:`${run.token} 连续 ${run.len} 题`,detail:`第 ${run.start+1}–${run.end+1} 题`,tag:'连续'});
  if(repeat)found.push({type:'repeat',start:repeat.start,end:repeat.end,title:`${repeat.block.join('')} 连续重复 ${repeat.reps} 轮`,detail:`第 ${repeat.start+1}–${repeat.end+1} 题`,tag:'重复'});
  for(const t of tokens){if((counts.get(t)||0)<2)continue;const g=longestGap(a,t),cut=quantile(gapSim.get(t),.97);if(g.len>=8&&g.len>=cut)found.push({type:'gap',start:g.start,end:g.end,title:`${t} 连续 ${g.len} 题没出现`,detail:g.start===0?`前 ${g.len} 题`:g.end===n-1?`最后 ${g.len} 题`:`第 ${g.start+1}–${g.end+1} 题`,tag:'空档'});if(w){const c=bestCluster(a,t,w),ccut=quantile(clusterSim.get(t),.98);if(c.count>=Math.ceil(w*.55)&&c.count>=ccut)found.push({type:'cluster',start:c.start,end:c.end,title:`${w} 题里有 ${c.count} 个 ${t}`,detail:`第 ${c.start+1}–${c.end+1} 题`,tag:'扎堆'})}}
...
function renderGrid(){if(!current)return;const a=current.answers,f=current.findings[selected]||null;const frag=document.createDocumentFragment();a.forEach((x,i)=>{const d=document.createElement('div');d.className='cell'+(f&&i>=f.start&&i<=f.end?' marked':'');d.dataset.i=String(i);d.innerHTML=`<i>${String(i+1).padStart(a.length>=100?3:2,'0')}</i><b>${esc(x)}</b>`;frag.appendChild(d)});grid.replaceChildren(frag);rangeText.textContent=`01–${a.length}`}
...
function chooseFinding(i){selected=i;renderFindings();renderGrid();const f=current?.findings[i];if(f){const cell=grid.querySelector(`[data-i="${f.start}"]`);cell?.scrollIntoView({block:'nearest',inline:'nearest',behavior:'smooth'})}}
