...
const restoreCanvas=$('#restoreCanvas'),restoreMeta=$('#restoreMeta'),widthInput=$('#width'),widthReset=$('#widthReset'),issues=$('#issues'),issuesMeta=$('#issuesMeta'),issueList=$('#issueList'),saveBtn=$('#save'),note=$('#note');
let pdfjs=null,pages=[],detection=null,measure=null,lastBuilt=null,forcedSide='auto',isReverse=false,runId=0;
...
async function readPages(file,id){
  const lib=await loadPdf();if(id!==runId)return null;const data=new Uint8Array(await file.arrayBuffer()),doc=await lib.getDocument({data}).promise;
  if(doc.numPages<2)throw new Error('few');if(doc.numPages>220)throw new Error('many');
  ...
  for(let i=1;i<=doc.numPages;i++){
    ...
    out.push({page:i,left:{image:leftImage,analysis:analyzeStrip(leftImage)},right:{image:rightImage,analysis:analyzeStrip(rightImage)}});canvas.width=1;canvas.height=1;
...
function renderCurrent(resetWidth=false){
  if(!pages.length)return;detection=chooseDetection(pages,forcedSide);measure=measurePages(pages,detection);const report=issueSummary(measure),found=measure.rows.filter(x=>x.found).length,coverage=found/pages.length;
  pageCount.textContent=String(pages.length);foundCount.textContent=String(found);issueCount.textContent=String(report.count);
  if(found<2||coverage<.16){verdict.textContent='没找到明显骑缝章';summary.textContent='可以换到另一边看看。';}
...
async function process(file){
  if(!file)return;if(file.type&&file.type!=='application/pdf'&&!/\.pdf$/i.test(file.name)){setError('请选择 PDF。');return}const id=++runId;setError('');home.hidden=true;work.hidden=false;busy.hidden=false;result.hidden=true;fileName.textContent=file.name;forcedSide='auto';isReverse=false;widthInput.value='100';pages=[];detection=measure=lastBuilt=null;
  try{const read=await readPages(file,id);if(!read||id!==runId)return;pages=read;busyTitle.textContent='正在拼';busyText.textContent='';await new Promise(r=>setTimeout(r,0));busy.hidden=true;result.hidden=false;renderCurrent(true);window.scrollTo({top:0,behavior:'instant'})}
  catch(e){console.error(e);work.hidden=true;home.hidden=false;if(e?.message==='few')setError('至少需要两页 PDF。');else if(e?.message==='many')setError('这份 PDF 超过 220 页，先拆成较小的一份再试。');else setError('这份 PDF 没有成功打开。')}
}
function setSide(side){forcedSide=side;renderCurrent(true)}
function setOrder(reverseValue){isReverse=reverseValue;renderCurrent(false)}
...
leftBtn.onclick=()=>setSide('left');rightBtn.onclick=()=>setSide('right');forward.onclick=()=>setOrder(false);reverse.onclick=()=>setOrder(true);widthInput.oninput=()=>renderCurrent(false);widthReset.onclick=()=>{widthInput.value='100';renderCurrent(false)};saveBtn.onclick=saveImage;
