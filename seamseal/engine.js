...
export function analyzeStrip(imageData,bins=192){
  const {data,width:w,height:h}=imageData,redHist=new Float32Array(bins),colorHist=new Float32Array(bins);
  let red=0,color=0;
  const step=h>950?2:1;
  for(let y=0;y<h;y+=step){const bin=Math.min(bins-1,Math.floor(y/h*bins));for(let x=0;x<w;x+=step){const i=(y*w+x)*4,r=data[i],g=data[i+1],b=data[i+2];const rs=ink(r,g,b,'red'),cs=ink(r,g,b,'color');if(rs){red+=rs;redHist[bin]+=rs}if(cs){color+=cs;colorHist[bin]+=cs}}}
...
export function chooseDetection(pages,forcedSide='auto'){
  ...
  const pageN=Math.max(1,pages.length);
  const score=s=>Math.max(stats[s].red*(.65+.35*stats[s].redPages/pageN),stats[s].color*(.52+.48*stats[s].colorPages/pageN));
  const side=forcedSide==='left'||forcedSide==='right'?forcedSide:(score('right')>=score('left')?'right':'left');
  const s=stats[side],redEnough=s.redPages>=Math.max(2,Math.ceil(pageN*.18))&&s.red>=Math.max(80,pageN*12);
  const mode=redEnough?'red':'color',hist=mode==='red'?s.redHist:s.colorHist,total=mode==='red'?s.red:s.color,pagesWith=mode==='red'?s.redPages:s.colorPages;
...
function bandMetrics(imageData,band,mode){
  const {data,width:w,height:h}=imageData,y0=clamp(Math.floor(band.start*h),0,h-1),y1=clamp(Math.ceil(band.end*h),y0+1,h),xs=[],ys=[];let amount=0;
  for(let y=y0;y<y1;y++){for(let x=0;x<w;x++){const i=(y*w+x)*4,s=ink(data[i],data[i+1],data[i+2],mode);if(s>.08){amount+=s;xs.push(x);ys.push(y)}}}
  if(!xs.length)return{amount:0,span:0,centerY:(y0+y1)/2,y0,y1};
  const minX=Math.min(...xs),maxX=Math.max(...xs);return{amount,span:maxX-minX+1,centerY:med(ys),y0,y1,minX,maxX};
}

function maskSignature(imageData,band,mode,side,depth=4){
...
function signatureSimilarity(a,b){
  if(!a?.length||!b?.length)return 0;let best=0;for(let shift=-6;shift<=6;shift++){let hit=0,union=0,n=0;for(let i=0;i<a.length;i++){const j=i+shift;if(j<0||j>=b.length)continue;const x=a[i],y=b[j];hit+=Math.min(x,y);union+=Math.max(x,y);n++}if(n&&union>0)best=Math.max(best,hit/union)}return best;
}
...
export function measurePages(pages,detection){
  ...
  for(const r of rows){r.found=r.amount>Math.max(7,amountMed*.13);r.weak=r.found&&r.amount<amountMed*.34}
  const seams=[];for(let i=0;i<rows.length-1;i++){const score=signatureSimilarity(rows[i].sig,rows[i+1].sig);seams.push({after:i+1,before:i+2,score})}
  const useful=seams.map(x=>x.score).filter(x=>x>0),sMed=med(useful),sLow=Math.max(.045,sMed*.34);
  for(const s of seams)s.suspicious=s.score<sLow;
  const autoWidth=clamp(Math.round(Math.max(8,spanMed+5)),8,Math.round(pages[0]?.[side]?.image?.width*.72||60));
...
function drawImageDataCrop(ctx,img,sx,sy,sw,sh,dx,dy,dw,dh,flipX=false){
  const c=document.createElement('canvas');c.width=img.width;c.height=img.height;c.getContext('2d').putImageData(img,0,0);
  if(flipX){ctx.save();ctx.translate(dx+dw,0);ctx.scale(-1,1);ctx.drawImage(c,sx,sy,sw,sh,0,dy,dw,dh);ctx.restore()}else ctx.drawImage(c,sx,sy,sw,sh,dx,dy,dw,dh);
}

export function buildReconstruction(pages,detection,measure,{reverse=false,widthScale=1}={}){
  const {side,band}=detection,src=[...pages];if(reverse)src.reverse();const baseW=clamp(Math.round(measure.autoWidth*widthScale),6,Math.round(pages[0][side].image.width*.85));
